...
func (p *ProxyETHEstimateGas) Request(rawreq *eth.JSONRPCRequest, c echo.Context) (interface{}, eth.JSONRPCError) {
	...
	// when supplying this parameter to callcontract to estimate gas in the sbit api
	...
	// sbit [code: -5] Incorrect address occurs here
	sbitresp, err := p.CallContract(sbitreq)
	if err != nil {
		return nil, eth.NewCallbackError(err.Error())
	}

	return p.toResp(sbitresp)
}

func (p *ProxyETHEstimateGas) toResp(sbitresp *sbit.CallContractResponse) (*eth.EstimateGasResponse, eth.JSONRPCError) {
	if sbitresp.ExecutionResult.Excepted != "None" {
		return nil, eth.NewCallbackError(ErrExecutionReverted.Error())
	}
	gas := eth.EstimateGasResponse(hexutil.EncodeUint64(uint64(float64(sbitresp.ExecutionResult.GasUsed) * GAS_BUFFER)))
