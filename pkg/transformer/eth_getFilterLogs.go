...
func (p *ProxyETHGetFilterLogs) request(filter *eth.Filter) (sbitresp eth.GetFilterChangesResponse, err eth.JSONRPCError) {
	...
	if !ok {
		return sbitresp, eth.NewCallbackError("Could not get toBlock")
	}
