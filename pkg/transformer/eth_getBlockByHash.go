...
func (p *ProxyETHGetBlockByHash) request(req *eth.GetBlockByHashRequest) (*eth.GetBlockByHashResponse, eth.JSONRPCError) {
	...
	if blockHeader.IsGenesisBlock() {
		...
		resp.Miner = utils.AddHexPrefix(sbit.ZeroAddress)
	} else {
	...
	resp.GasLimit = utils.AddHexPrefix(sbit.DefaultBlockGasLimit)
	resp.GasUsed = "0x0"
	...
	if req.FullTransaction {
		for _, txHash := range block.Txs {
			tx, err := getTransactionByHash(p.Sbit, txHash)
			if err != nil {
			...
			if tx == nil {
				if block.Height == 0 {
					...
					if !p.GetFlagBool(sbit.FLAG_IGNORE_UNKNOWN_TX) {
		...
		for _, txHash := range block.Txs {
			...
			// 	has [64]byte just like Sbit tx hash has. In this case we do no
