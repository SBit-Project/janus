...
func TestGetTransactionByHashRequest(t *testing.T) {
	...
	proxyEth := ProxyETHGetTransactionByHash{sbitClient}
...
func TestGetTransactionByHashRequestWithContractVout(t *testing.T) {
	...
	internal.SetupGetBlockByHashResponsesWithVouts(
		...
		[]*sbit.DecodedRawTransactionOutV{
			{
				Value: decimal.Zero,
				N:     0,
				ScriptPubKey: sbit.DecodedRawTransactionScriptPubKey{
...
/*
// TODO: Removing this unit test as the transformer computes the "Amount" value (how much SBIT was transferred out) from the MethodDecodeRawTransaction response
...
func TestGetTransactionByHashRequest_PrecisionOverflow(t *testing.T) {
	...
	sbitClient, err := createMockedClient(mockedClientDoer)
	...
	err = mockedClientDoer.AddResponseWithRequestID(2, sbit.MethodGetTransaction, getTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}
	...
	decodedRawTransactionResponse := sbit.DecodedRawTransactionResponse{
		...
		Vouts: []*sbit.DecodedRawTransactionOutV{},
	}
	err = mockedClientDoer.AddResponseWithRequestID(3, sbit.MethodDecodeRawTransaction, decodedRawTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = mockedClientDoer.AddResponseWithRequestID(4, sbit.MethodGetBlock, getBlockResponse)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = mockedClientDoer.AddResponseWithRequestID(4, sbit.MethodGetRawTransaction, &getRawTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}
