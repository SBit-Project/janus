...
func TestMultipleLogsWithORdTopics(t *testing.T) {
	...
	//Add response
	clientDoerMock.AddRawResponse(sbit.MethodSearchLogs, []byte(rawResponse))

	//Prepare proxy & execute
	//preparing proxy & executing
	proxyEth := ProxyETHGetLogs{sbitClient}
...
func testGetLogsWithTopics(t *testing.T, topics []interface{}, want eth.GetLogsResponse) {
	...
	//Add response
	err = clientDoerMock.AddResponseWithRequestID(2, sbit.MethodSearchLogs, searchLogsResponse)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetLogsTranslateTopicWorksWithNil(t *testing.T) {
	...
	sbitRequest, jsonErr := proxyEth.ToRequest(&request)
	if jsonErr != nil {
		t.Fatal(jsonErr)
	}

	sbitRawRequest, err := json.Marshal(sbitRequest)
	if err != nil {
		t.Fatal(err)
	}
	...
	internal.CheckTestResultEthRequestLog(request, expectedRawRequest, string(sbitRawRequest), t, false)
}
