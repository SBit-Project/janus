...
func TestEthCallRequest(t *testing.T) {
	...
	sbitClient, err := internal.CreateMockedClient(clientDoerMock)
	...
	err = clientDoerMock.AddResponseWithRequestID(1, sbit.MethodCallContract, callContractResponse)
	if err != nil {
		t.Fatal(err)
	}

	fromHexAddressResponse := sbit.FromHexAddressResponse("0x1e6f89d7399081b4f8f8aa1ae2805a5efff2f960")
	err = clientDoerMock.AddResponseWithRequestID(2, sbit.MethodFromHexAddress, fromHexAddressResponse)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRetry(t *testing.T) {
	...
	// return SBIT is busy response 4 times
	for i := 0; i < 4; i++ {
		clientDoerMock.AddRawResponse(sbit.MethodCallContract, []byte(sbit.ErrSbitWorkQueueDepth.Error()))
	}
	// on 5th request, return correct value
	clientDoerMock.AddResponseWithRequestID(1, sbit.MethodCallContract, callContractResponse)
...
func TestEthCallRequestOnUnknownContract(t *testing.T) {
	...
	fromHexAddressResponse := sbit.FromHexAddressResponse("0x1e6f89d7399081b4f8f8aa1ae2805a5efff2f960")
	err = clientDoerMock.AddResponse(sbit.MethodFromHexAddress, fromHexAddressResponse)
	if err != nil {
		t.Fatal(err)
	}

	//preparing error response
	unknownAddressResponse := sbit.GetErrorResponse(sbit.ErrInvalidAddress)
	err = clientDoerMock.AddError(sbit.MethodCallContract, unknownAddressResponse)
	if err != nil {
		t.Fatal(err)
	}
