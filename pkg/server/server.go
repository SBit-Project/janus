...
import (
	...
	"github.com/SBit-Project/janus/pkg/transformer"
...
func (s *Server) Start() error {
	...
	health.AddLivenessCheck("sbitd-connection", func() error { return s.testConnectionToSbitd() })
	health.AddLivenessCheck("sbitd-logevents-enabled", func() error { return s.testLogEvents() })
	health.AddLivenessCheck("sbitd-blocks-syncing", func() error { return s.testBlocksSyncing() })
	...
	e.Use(middleware.BodyDump(func(c echo.Context, req []byte, res []byte) {
		...
		if s.debug {
			reqBody, reqErr := sbit.ReformatJSON(req)
			resBody, resErr := sbit.ReformatJSON(res)
	...
	// TODO: Upgrade golang to 1.15 to support s.sbitRPCClient.GetURL().Redacted() here
	url := s.sbitRPCClient.URL
	level.Info(s.logger).Log("listen", s.address, "sbit_rpc", url, "msg", "proxy started", "https", https)
