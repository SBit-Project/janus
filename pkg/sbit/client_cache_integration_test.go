...
func TestCacheWithClient(t *testing.T) {
	sbitMockServer := NewSbitMockServer(mockJsonRPCResponse)
	URL := "http://sbituser:sbitpass@127.0.0.1:6969"
	defer sbitMockServer.Close()
	...
	client.URL = sbitMockServer.URL
	if err != nil {
		t.Fatal(err)
	}

	var result interface{}
	t.Run("Sbit RPC getblock call processed succesfully", func(t *testing.T) {
	...
	t.Run("Upon second getblock call, response is returned from cache", func(t *testing.T) {
		...
		if !strings.Contains(outputLog, "sbit (CACHED) RPC response") {
			t.Errorf("\nexpected: %s\n\n, got: %s", "sbit (CACHED) RPC response", outputLog)
		}
...
func NewSbitMockServer(body []byte) *httptest.Server {
