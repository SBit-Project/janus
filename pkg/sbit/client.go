...
var FLAG_HIDE_SBITD_LOGS = "HIDE_SBITD_LOGS"
...
func (c *Client) RequestWithContext(ctx context.Context, method string, params interface{}, result interface{}) error {
	...
	if c.cache.IsCachable(method) {
		...
		if cachedResult != nil && err == nil {
			...
			if c.IsDebugEnabled() && !c.GetFlagBool(FLAG_HIDE_SBITD_LOGS) {
	...
	for i := 0; i < max; i++ {
		...
		if err != nil {
			if strings.Contains(err.Error(), ErrSbitWorkQueueDepth.Error()) && i != max-1 {
				requestString := marshalToString(req)
				backoffTime := computeBackoff(i, true)
				c.GetLogger().Log("msg", fmt.Sprintf("SBIT process busy, backing off for %f seconds", backoffTime.Seconds()), "request", requestString)
				time.Sleep(backoffTime)
				c.GetLogger().Log("msg", "Retrying SBIT command")
			} else {
				if i != 0 {
					c.GetLogger().Log("msg", fmt.Sprintf("Giving up on SBIT RPC call after %d tries since its busy", i+1))
				}
				return err
			}
		} else {
...
func (c *Client) Do(ctx context.Context, req *JSONRPCRequest) (*SuccessJSONRPCResult, error) {
	...
	if c.IsDebugEnabled() && !c.GetFlagBool(FLAG_HIDE_SBITD_LOGS) && c.logWriter != nil {
		fmt.Fprintf(c.logWriter, "=> sbit RPC request\n%s\n", reqBody)
	}
	...
	if c.IsDebugEnabled() && !c.GetFlagBool(FLAG_HIDE_SBITD_LOGS) {
		...
		if err == nil && c.logWriter != nil {
			fmt.Fprintf(c.logWriter, "<= sbit RPC response\n%s\n", formattedBodyStr)
		}
	}
	...
	if err != nil {
		...
		if string(respBody) == ErrSbitWorkQueueDepth.Error() {
			// SBIT http server queue depth reached, need to retry
			return nil, ErrSbitWorkQueueDepth
		}
...
func (c *Client) do(ctx context.Context, body io.Reader) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "ioutil error in sbit client package")
	}
	return reader, nil
}
...
func SetLogger(l log.Logger) func(*Client) error {
	return func(c *Client) error {
		c.logger = log.WithPrefix(l, "component", "sbit.Client")
		return nil
	}
}
...
func SetDisableSnippingSbitRpcOutput(disable bool) func(*Client) error {
	return func(c *Client) error {
...
func SetHideSbitdLogs(hide bool) func(*Client) error {
	return func(c *Client) error {
		c.SetFlag(FLAG_HIDE_SBITD_LOGS, hide)
		return nil
	}
}
...
func checkRPCURL(u string) error {
	...
	sbitRPC, err := url.Parse(u)
	if err != nil {
		return errors.Errorf("SBIT_RPC URL: %s", u)
	}

	if sbitRPC.User == nil {
		return errors.Errorf("SBIT_RPC URL (must specify user & password): %s", u)
	}
...
func (c *Client) printCachedRPCResponse(cachedResponse []byte) {
	...
	if err == nil && c.logWriter != nil {
		fmt.Fprintf(c.logWriter, "<= sbit (CACHED) RPC response\n%s\n", formattedBodyStr)
	}
}
