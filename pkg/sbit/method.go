...
// hard coded 0x1 due to the unique nature of Sbits UTXO system, might
...
func (m *Method) Generate(blockNum int, maxTries *int) (resp GenerateResponse, err error) {
	...
	if len(m.Accounts) == 0 && generateToAccount == nil {
		return nil, errors.New("you must specify SBIT accounts")
	}
	...
	if generateToAccount == nil {
		...
		sAddress, err = acc.ToBase58Address(m.isMain)
		if err != nil {
			if m.IsDebugEnabled() {
		...
		m.GetDebugLogger().Log("function", "Generate", "msg", "generating to account 0", "account", sAddress)
	} else {
		sAddress = *generateToAccount
		m.GetDebugLogger().Log("function", "Generate", "msg", "generating to specified account", "account", sAddress)
	}
...
/**
 * Note that SBIT searchlogs api returns all logs in a transaction receipt if any log matches a topic
