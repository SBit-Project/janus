...
func (r *SendToAddressRequest) MarshalJSON() ([]byte, error) {
	/*
		1. "address"            (string, required) The sbit address to send to.
		2. "amount"             (numeric or string, required) The amount in SBIT to send. eg 0.1
		...
		5. subtractfeefromamount  (boolean, optional, default=false) The fee will be deducted from the amount being sent.
		                             The recipient will receive less sbits than you enter in the amount field.
...
func (r *SendToContractRequest) MarshalJSON() ([]byte, error) {
	/*
	   ...
	   3. "amount"      (numeric or string, optional) The amount in SBIT to send. eg 0.1, default: 0
	   4. gasLimit  (numeric or string, optional) gasLimit, default: 250000, max: 40000000
	   5. gasPrice  (numeric or string, optional) gasPrice Sbit price per gas unit, default: 0.0000004, min:0.0000004
...
func (r *CreateContractRequest) MarshalJSON() ([]byte, error) {
	/*
		...
		3. gasPrice  (numeric or string, optional) gasPrice SBIT price per gas unit, default: 0.0000004, min:0.0000004
...
// Calculates transaction total amount of Sbit
	...
	for _, vout := range resp.Vouts {
		...
		if strings.HasSuffix(vout.ScriptPubKey.ASM, "OP_CREATE") || strings.HasSuffix(vout.ScriptPubKey.ASM, "OP_CALL") {
			...
			if isOpCreateWithOpSender || isOpCallWithOpSender {
				...
				// TODO: Is it necessary to check that the first three ASM entries are valid, as done in SbitJ?

				// TODO: These following sanity checks are present in the SbitJ code used as reference, but will probably always pass for valid blockchain data.
...
func (r *GetRawTransactionResponse) GetMiningFeeInSBIT() float64 {
...
type (
	...
	/*
		[
			"sUbxboqjBRp96j3La8D1RYkyqx5uQbJPoW"
		]
	*/
...
type (
	/*
		     ...
		     [
		       {                              (json object)
		         "address": amount,           (numeric or string, required) A key-value pair. The key (string) is the sbit address, the value (float or string) is the amount in SBIT
		       },
		       {                              (json object)
		       ...
		       {                              (json object) (send to contract)
		         ...
		         "amount": amount,            (numeric or string, optional, default=0) Value in SBIT to send with the call, should be a valid amount, default 0
		         "gasLimit": n,               (numeric) The gas limit for the transaction
		         "gasPrice": n,               (numeric) The gas price for the transaction
		         "senderaddress": "hex",      (string) The sbit address that will be used to create the contract.
		       },
...
type (
	/*
		...
		1. Input params              (json object, required) Json object
			{
			"addresses": [        (json array, required) The sbit addresses
				"address",          (string) The sbit address
				...
			],
...
type (
	...
	/*
		...
		3. "addresses"      (string) A json array of sbit addresses to filter
		    [
		      "address"     (string) sbit address
		      ,...
		    ]
		...
		5. query_options    (json, optional) JSON with query options
		    {
		      "minimumAmount"    (numeric or string, default=0) Minimum value of each UTXO in SBIT
		      "maximumAmount"    (numeric or string, default=unlimited) Maximum value of each UTXO in SBIT
		      "maximumCount"     (numeric or string, default=unlimited) Maximum number of UTXOs
		      "minimumSumAmount" (numeric or string, default=unlimited) Minimum sum value of all UTXOs in SBIT
		    }
	*/
				...
				[                   (array of json object)
					{
						...
						"address" : "address",    (string) the sbit address
						"account" : "account",    (string) DEPRECATED. The associated account, or "" for the default account
						"scriptPubKey" : "key",   (string) the script key
						"amount" : x.xxx,         (numeric) the transaction output amount in SBIT
		...
		[
			{
				...
				"vout": 2,
				"address": "sUbxboqjBRp96j3La8D1RYkyqx5uQbJPoW",
				"account": "",
...
type (
	...
	/*
		Arguments:
		1. addresses       	(json array, required) The sbit addresses
			[
				"address",	(string) The sbit address
				...
			]
