...
func NewAgent(ctx context.Context, sbit *sbit.Sbit, transformer Transformer) *Agent {
	return newAgentWithConfiguration(ctx, sbit, transformer, make(map[string]interface{}))
}

func newAgentWithConfiguration(ctx context.Context, sbit *sbit.Sbit, transformer Transformer, configuration map[string]interface{}) *Agent {
	if ctx == nil {
		panic("ctx cannot be nil")
	}
	if sbit == nil {
		panic("sbit cannot be nil")
	}
	agent := &Agent{
		sbit:          sbit,
...
func (a *Agent) run() {
	...
	defer func() {
		...
		a.sbit.GetDebugLogger().Log("msg", "Agent exited subscription processing thread")
	...
	a.sbit.GetDebugLogger().Log("msg", "Agent started subscription processing thread")
	...
	for {
		...
		a.mutex.RUnlock()
		if transformer == nil {
			a.sbit.GetErrorLogger().Log("msg", "Agent does not have access to eth transformer, cannot process 'newHeads' subscriptions")
		} else {
			blockchainInfo, err := a.sbit.GetBlockChainInfo()
			if err != nil {
				a.sbit.GetErrorLogger().Log("msg", "Failure getting blockchaininfo", "err", err)
			} else {
				latestBlock := blockchainInfo.Blocks
				if lastBlock == 0 {
					// prevent sending the current head to the first client connected
					lastBlock = latestBlock
					a.sbit.GetDebugLogger().Log("msg", "Got getblockchaininfo response for same block", "block", lastBlock)
				} else if latestBlock > lastBlock {
					a.sbit.GetDebugLogger().Log("msg", "New head detected", "block", latestBlock)
					...
					if jsonErr != nil {
						a.sbit.GetErrorLogger().Log("msg", "Failed to eth_getBlockByHash", "hash", blockchainInfo.Bestblockhash, "err", jsonErr)
					} else {
						getBlockByHashResponse, ok := result.(*eth.GetBlockByHashResponse)
						if !ok {
							a.sbit.GetErrorLogger().Log("msg", "Failed to eth_getBlockByHash, unexpected response type", "hash", blockchainInfo.Bestblockhash)
						} else {
							lastBlock = latestBlock
							// notify newHead
					...
					a.sbit.GetDebugLogger().Log("msg", "Detected same head", "block", latestBlock)
				}
			}
		}
