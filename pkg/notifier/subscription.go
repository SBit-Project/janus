...
func (s *subscriptionInformation) run() {
	...
	if err != nil {
		s.sbit.GetDebugLogger().Log("msg", "Error translating logs topics", "error", err)
		return
	}
	ethAddresses, err := s.params.Params.GetAddresses()
	if err != nil {
		s.sbit.GetDebugLogger().Log("msg", "Error translating logs addresses", "error", err)
		return
	}
	...
	sbitTopics := sbit.NewSearchLogsTopics(translatedTopics)
	req := &sbit.WaitForLogsRequest{
		FromBlock: nextBlock,
		ToBlock:   nil,
		Filter: sbit.WaitForLogsFilter{
			Addresses: &stringAddresses,
			Topics:    &sbitTopics,
		},
	}

	if s.sbit.Chain() == sbit.ChainRegTest || s.sbit.Chain() == sbit.ChainTest {
		req.MinimumConfirmations = 0
	}

	// this throttles SBIT api calls if waitforlogs is returning very quickly a lot
	limitToXApiCalls := 5
	inYSeconds := 10 * time.Second
	// if a SBIT API call returns quicker than this, we will wait until this time is reached
	// this prevents spamming the SBIT node too much
	...
	for {
		...
		resp, err := s.sbit.WaitForLogsWithContext(s.ctx, req)
		timeAfterCall := time.Now()
		if err == nil {
			nextBlock = int(resp.NextBlock)
			reqSearchLogs := sbit.SearchLogsRequest{
			...
			receiptsSearchLogs, err := s.sbit.SearchLogs(&reqSearchLogs)
			if err != nil {
				s.sbit.GetErrorLogger().Log("msg", "Error calling searchLogs", "subscriptionId", s.id, "error", err)
				return
			}
			for _, sbitLog := range receiptsSearchLogs {
				sbitLogs := sbitLog.Log
				logs := conversion.FilterSbitLogs(stringAddresses, sbitTopics, sbitLogs)
				ethLogs := conversion.ExtractETHLogsFromTransactionReceipt(sbitLog, logs)
				for _, ethLog := range ethLogs {
					...
					if _, ok := sentHashes[hash]; !ok {
						...
						s.sbit.GetDebugLogger().Log("subscriptionId", s.id, "msg", "notifying of logs")
						jsonRpcNotification, err := eth.NewJSONRPCNotification("eth_subscription", subscription)
						if err != nil {
							s.sbit.GetErrorLogger().Log("subscriptionId", s.id, "err", err)
							return
						}
			...
			// error occurred
			s.sbit.GetDebugLogger().Log("subscriptionId", s.id, "err", err)
			failures = failures + 1
		}
		...
		case <-done:
			...
			s.sbit.GetDebugLogger().Log("subscriptionId", s.id, "msg", "context closed, dropping subscription")
			return
		default:
		}
		...
		if backoffTime > 0 {
			s.sbit.GetDebugLogger().Log("subscriptionId", s.id, "msg", fmt.Sprintf("backing off for %d miliseconds", backoffTime/time.Millisecond))
		}
