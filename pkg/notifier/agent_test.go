...
func TestAgentAddSubscriptionLogs(t *testing.T) {
	...
	doer.AddResponse(sbit.MethodWaitForLogs, sbit.WaitForLogsResponse{
		Entries: []sbit.WaitForLogsEntry{
			internal.SbitWaitForLogsEntry(sbit.Log{
				Address: internal.SbitTransactionReceipt(nil).ContractAddress,
		...
		Count:     1,
		NextBlock: internal.SbitTransactionReceipt(nil).BlockNumber + 1,
	})

	doer.AddResponse(
		sbit.MethodSearchLogs,
		sbit.SearchLogsResponse{
			internal.SbitTransactionReceipt(
				[]sbit.Log{
					{
						Address: internal.SbitTransactionReceipt(nil).ContractAddress,
						Topics:  []string{topic1},
...
func TestAgentAddSubscriptionNewHeads(t *testing.T) {
	...
	for i := int64(1); i < 10; i++ {
		doer.AddResponse(sbit.MethodGetBlockChainInfo, sbit.GetBlockChainInfoResponse{
			Blocks:        i,
			Bestblockhash: "0x1",
		})
	}
