...
func CreateMockedClient(doerInstance Doer) (sbitClient *sbit.Sbit, err error) {
	...
	sbitJSONRPC, err := sbit.NewClient(
		...
		sbit.SetDoer(doerInstance),
		sbit.SetDebug(isDebugEnvironmentVariableSet()),
	...
	sbitClient, err = sbit.New(sbitJSONRPC, "test")
	return
}
...
var (
	...
	GetTransactionByHashResponseWithTransactions = eth.GetBlockByHashResponse{
		...
		GasLimit:         utils.AddHexPrefix(sbit.DefaultBlockGasLimit),
		GasUsed:          "0x0",
	...
	GetBlockResponse = sbit.GetBlockResponse{
...
func SbitTransactionReceipt(logs []sbit.Log) sbit.TransactionReceipt {
	return sbit.TransactionReceipt{
		BlockHash:         GetTransactionByHashBlockHexHash,
...
func SbitWaitForLogsEntry(log sbit.Log) sbit.WaitForLogsEntry {
	return sbit.WaitForLogsEntry{
		BlockHash:         GetTransactionByHashBlockHexHash,
		BlockNumber:       GetTransactionByHashBlockNumberInteger,
		TransactionHash:   GetTransactionByHashResponseData.Hash,
		TransactionIndex:  hexutil.MustDecodeUint64(GetTransactionByHashResponseData.TransactionIndex),
...
func SetupGetBlockByHashResponses(t *testing.T, mockedClientDoer Doer) {
	SetupGetBlockByHashResponsesWithVouts(t, []*sbit.DecodedRawTransactionOutV{}, mockedClientDoer)
}

func SetupGetBlockByHashResponsesWithVouts(t *testing.T, vouts []*sbit.DecodedRawTransactionOutV, mockedClientDoer Doer) {
	//preparing answer to "getblockhash"
	getBlockHashResponse := sbit.GetBlockHashResponse(GetTransactionByHashBlockHexHash)
	err := mockedClientDoer.AddResponse(sbit.MethodGetBlockHash, getBlockHashResponse)
	if err != nil {
		t.Fatal(err)
	}

	getBlockHeaderResponse := sbit.GetBlockHeaderResponse{
		Hash:              GetTransactionByHashBlockHash,
		Confirmations:     1,
		Height:            3983,
		Version:           536870912,
	...
	err = mockedClientDoer.AddResponse(sbit.MethodGetBlockHeader, getBlockHeaderResponse)
	if err != nil {
		t.Fatal(err)
	}

	err = mockedClientDoer.AddResponse(sbit.MethodGetBlock, GetBlockResponse)
	if err != nil {
		t.Fatal(err)
	}

	getTransactionResponse := sbit.GetTransactionResponse{
		...
		Details: []*sbit.TransactionDetail{{Account: "",
			Category:  "send",
	...
	err = mockedClientDoer.AddResponse(sbit.MethodGetTransaction, getTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}

	decodedRawTransactionResponse := sbit.DecodedRawTransactionResponse{
		ID:       "11e97fa5877c5df349934bafc02da6218038a427e8ed081f048626fa6eb523f5",
		...
		Version:  2,
		Locktime: 608,
		Vins: []*sbit.DecodedRawTransactionInV{{
			TxID: "7f5350dc474f2953a3f30282c1afcad2fb61cdcea5bd949c808ecc6f64ce1503",
			Vout: 0,
			ScriptSig: sbit.DecodedRawTransactionScriptSig{
	...
	err = mockedClientDoer.AddResponse(sbit.MethodDecodeRawTransaction, decodedRawTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}

	getTransactionReceiptResponse := sbit.GetTransactionReceiptResponse{}
	err = mockedClientDoer.AddResponse(sbit.MethodGetTransactionReceipt, &getTransactionReceiptResponse)
	if err != nil {
		t.Fatal(err)
	}

	// TODO: Get an actual response for this (only addresses are used in this test though)
	getRawTransactionResponse := sbit.GetRawTransactionResponse{
		Vins: []sbit.RawTransactionVin{
			{
				Address: "QXeZZ5MsAF5pPrPy47ZFMmtCpg7RExT4mi",
			},
		},
		Vouts: []sbit.RawTransactionVout{
			{
				Details: struct {
	...
	err = mockedClientDoer.AddResponse(sbit.MethodGetRawTransaction, &getRawTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}
}
...
// TODO: Handle unexported struct fields, like in TestEthValueToSbitAmount (pkg/transformer/util_test)
